...
def k_largest_component_roots(N,k = 2):
...
def N_component_masks(N,roots):
    # make a mask for each root
    for i in range(len(roots)):
        # initialise mask of vertices
        mask = np.zeros_like(N.graph.get_vertices())
        # do dfs from root returning array
        verts = gt.bfs_iterator(N.graph,roots[i], array = True)
        # set of verts in edges

        # update mask
        mask[np.unique(verts)] = 1
        # name and bind to graph
        N.graph.vp['k_' + str(i+1)] = N.graph.new_vp('bool', mask)