...
"""Returns a vertex property map indicating reachable leaf nodes from each vertex.

Leaf reachability is computed by finding all paths from the root vertex to leaf vertices, 
then checking which vertices are contained in those paths.
...
internalise : bool, optional
    If True, store output as internal vertex property 'reachable_leaves'. 
    If False (default), return vertex property map directly.

Returns
-------
vprop_rl : gt.VertexPropertyMap (if internalise=False)
    Vertex property map of reachable leaf counts
"""