...
def find_row_indices(n: np.ndarray, m: np.ndarray) -> np.ndarray:
    ...
    Returns:
      - indices: an (M,) integer array so that n[indices[i]] == m[i]
    """
    n = np.asarray(n)
    m = np.asarray(m)
    ...
    # 2) View and flatten
    n_view = np.ascontiguousarray(n).view(dtype_void).ravel()
    ...
    pos_in_sorted = np.searchsorted(n_sorted, m_view)
...
# make subtree mask
def g_subtree_mask(N: Tree_graph, root: int, bind: bool = True) -> tuple[gt.VertexPropertyMap, gt.EdgePropertyMap]:
