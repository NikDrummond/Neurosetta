...
def voxel_grid_downsample(points: jnp.ndarray, voxel_size: float) -> jnp.ndarray:
    ...
    # Hash the voxel indices into a unique 1D index
    ...
    # Sum positions and count per voxel
...
def estimate_alpha(pnts, percentile = 1):
    dists = pdist(filtered_points)
alpha_guess = np.percentile(dists, 1)  # e.g. 1st percentile
