...
def align_neuron(N:nr.Tree_graph, synapses:bool = True, scale:bool = True, s: int|float|np.ndarray|NoneType = None, invert:bool = True, bind:bool = True, unpack:bool = True, **kwargs) -> list | np.ndarray | nr.Tree_graph:
    ...
    **kwargs
    ...
    v_coords = nr.g_vert_coords(N)
    # check if we have input and output properties as we will need to handle them as well
    if synapses:
        assert nr.g_has_property(N,'inputs'), 'Input neuron must have input synapses to transform synapses'
        assert nr.g_has_property(N,'outputs'), 'Input neuron must have output synapses to transform synapses'
    ...
    all_coords = nr.snap_to_axis(all_coords).T

    # if we want to scale
    if scale:
        assert scale != None, 'If you wish to scale coordinates, s must be provided. See scale function documentation for details'
        all_coords = nr.coordinate_scale(all_coords,s,invert)
