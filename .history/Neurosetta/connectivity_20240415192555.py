...
def get_edge_coords(N):
    ## Get coordinates of start and end point of each edge in the graph
    # edges
    edges = N.graph.get_edges()
    coords = nr.g_vert_coords(N)
    # coordinates of edges
    p1 = np.zeros((edges.shape[0],3))
    p2 = np.zeros_like(p1)
    for i in range(len(edges)):
        p1[i] = coords[edges[i,0]]
        p2[i] = coords[edges[i,1]]

    return p1, p2    


def map_synapses(N:nr.Tree_graph,df:pd.DataFrame,inputs:bool = True, outputs:bool = True,threshold: float = 4000) -> nr.Tree_graph:
    """
    
    """

    # split given data frame into inputs and outputs
    in_df = syn_df.loc[syn_df.post == int(N.name),['pre','post_x','post_y','post_z','cleft_score','id']]
    out_df = syn_df.loc[syn_df.pre == int(N.name),['post','pre_x','pre_y','pre_z','cleft_score','id']]
