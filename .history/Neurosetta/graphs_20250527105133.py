...
def get_sub_tree_quality(N: nr.Tree_graph, v: int, total_cable: float = None, total_leaves: float = None, l_inds:np.ndarray = None) -> float:
    ...
    total_cable = nr.g_cable_length(N) if total_cable is None else total_cable
    total_leaves = len(nr.g_leaf_inds(N)) if total_leaves is None else total_leaves
    l_inds = nr.g_leaf_inds(N) if l_inds is None else l_inds
