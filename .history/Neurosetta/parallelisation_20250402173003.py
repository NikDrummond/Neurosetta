...
def Random_ST_cable_distribution_pop(N_all: List | Forest_graph, perms: int = 100) -> np.ndarray:
    ...
    if isinstance(N_all, List):
        ...
        coords = g_vert_coords(N_all.graph.vp['Neurons'][v])
    else:
        raise TypeError('N_all input must be List or Forest_graph')
