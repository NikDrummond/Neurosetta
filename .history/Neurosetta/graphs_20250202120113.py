...
def node_asymmetry(N: nr.Tree_graph | gt.Graph, L: List | np.ndarray | None = None, weight = True, bind = True) -> gt.VertexPropertyMap | None:
    ...
    asymmetries = N.graph.new_vp('double', np.ones_like(N.graph.get_vertices()))
    # iterate through internal nodes
    for v in nr.g_branch_inds(N):
        asymmetries[v] = node_asymmetry(N_all.graph.vp['Neurons'][0],v, L,weight = weight)
...
def expected_asymmetry(N: nr.Tree_graph | gt.Graph | gt.VertexPropertyMap,method: str = 'mean') -> float :
    """
    ...
    if nr.g_has_property(g,'Weighted_asymmetry'):
        arr = g.vp['Weighted_asymmetry'].a
    elif nr.g_has_property(g, 'Asymmetry'):
