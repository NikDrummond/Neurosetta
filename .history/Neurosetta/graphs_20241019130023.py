...
def synapse_MST(N:nr.Tree_graph, synapses:str = 'All', root: bool = True) -> nr.Tree_graph:

    assert isinstance(N,nr.Tree_graph), "Input must be neurosetta.Tree_graph"

    ### Get coordinates according to include
    if synapses == 'Inputs':
        assert nr.g_has_property(N,'inputs'), "Given neuron must have inputs property to make MST of Input Synapses"
        coords = N.graph.gp['inputs'][['graph_x','graph_y','graph_z']].values
    elif synapses == 'Output':
        assert nr.g_has_property(N,'outputs'), "Given neuron must have outputs property to make MST of Output Synapses"
        coords = N.graph.gp['outputs'][['graph_x','graph_y','graph_z']].values
    elif synapses == 'All':
        assert (nr.g_has_property(N,'inputs') and nr.g_has_property(N,'outputs')), "Given neuron must have both outputs and inputs property to make MST of All Synapses"
