...
def plot3d(N, radius = False, **kwargs):
    """
...
def _vd_tree_lines(N, **kwargs):

    start_pts,end_pts = _vd_tree_st_end(N)

    # collection of lines for plotting
    lns = vd.Lines(start_pts = start_pts,
                    end_pts = end_pts, **kwargs)
    return lns


def _vd_tree_cyl(N,**kwargs):
    ...
    tubes = [vd.Cylinder(pos = [start_pts[i],end_pts[i]],r = radii[i],**kwargs) for i in range(start_pts.shape[0])]

    return tubes

def _vd_nodes_lines(N,**kwargs):
...
def _vd_nodes_cyl(N,**kwargs):
    start_pts, end_pts, bool_ind = _vd_nodes_st_end(N)
    # create a list of cylinders
    radii = N.nodes.radius.values
    radii = radii[bool_ind]
    # create list of cylinders
    tubes = [vd.Cylinder(pos = [end_pts[i],start_pts[i]],r = radii[i],**kwargs) for i in range(start_pts.shape[0])]

    return tubes

def _vd_mesh(N,**kwargs):
    # generate vedo mesh
